...
class CustomAgent(BaseChatAgent, Component[CustomAgentConfig]):
    """A custom agent template for Autogen."""

    component_config_schema = CustomAgentConfig
    DEFAULT_DESCRIPTION = "A custom agent for handling specific tasks."
    ...
    def _to_config(self) -> CustomAgentConfig:
        ...
        Returns:
            CustomAgentConfig: The configuration for the agent.
        """
    ...
    def _from_config(cls, config: CustomAgentConfig) -> Self:
        ...
        Args:
            config (CustomAgentConfig): The configuration for the agent.
